...
describe("getScrewdriverPipelineByRepo", () => {
  ...
  beforeEach(() => {
    (getScrewdriverPipelineByRepoFromDomain as jest.Mock).mockResolvedValue(mockPipeline);
  });
  ...
  it("should return pipeline data from the domain layer", async () => {
    const result = await getScrewdriverPipelineByRepo.handler({ orgName: mockOrgName, repoName: mockRepoName, jwtToken: mockJwt });
    expect(result.content[0].text).toEqual(JSON.stringify(mockPipeline));
    expect(getScrewdriverPipelineByRepoFromDomain).toHaveBeenCalledWith(mockOrgName, mockRepoName, mockJwt);
  });

  it("should throw an error if the domain layer throws an error", async () => {
    const mockError = new Error("Domain error");
    (getScrewdriverPipelineByRepoFromDomain as jest.Mock).mockRejectedValue(mockError);

    await expect(getScrewdriverPipelineByRepo.handler({ orgName: mockOrgName, repoName: mockRepoName, jwtToken: mockJwt })).rejects.toThrow(
      "Domain error"
    );
    expect(getScrewdriverPipelineByRepoFromDomain).toHaveBeenCalledWith(mockOrgName, mockRepoName, mockJwt);
  });

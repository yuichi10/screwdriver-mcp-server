...
describe("getScrewdriverPipelineByRepo", () => {
  ...
  beforeEach(() => {
    (getScrewdriverPipelineByRepoFromApi as jest.Mock).mockResolvedValue(mockPipeline);
  });
  ...
  it("should return pipeline data from the infrastructure layer", async () => {
    ...
    expect(getScrewdriverPipelineByRepoFromApi).toHaveBeenCalledWith(mockOrgName, mockRepoName, mockJwt);
  });

  it("should throw an error if the infrastructure layer throws an error", async () => {
    const mockError = new Error("Infrastructure error");
    (getScrewdriverPipelineByRepoFromApi as jest.Mock).mockRejectedValue(mockError);
